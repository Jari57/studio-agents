...
const phases = [
  { title: '1. Write the brief', icon: Music, color: '#a78bfa', body: 'Choose a tempo, mood, instrumentation and structure. Keep your own lyrics and exclusions explicit. Generate an instrumental, listen to the entire return, then save a version.' },
  { title: '2. Build the performance', icon: Mic2, color: '#f472b6', body: 'Upload a vocal you own or use a supported musical-generation provider. Spoken narration is not singing. Voice identity requires permission and a compatible reference workflow; it is not guaranteed by a voice preset.' },
  { title: '3. Arrange and mix', icon: Sliders, color: '#22d3ee', body: 'Use the producer canvas to align clips, set levels, trim timing and export a mix. Review transitions, intelligibility and headroom. A mixed song is not an isolated vocal stem.' },
  { title: '4. Create the visual package', icon: Image, color: '#fbbf24', body: 'Generate artwork and inspect faces, lettering, composition and usage rights. Test the cover at thumbnail size. Keep every useful version in the same project.' },
  { title: '5. Review motion', icon: Video, color: '#818cf8', body: 'Generate short video clips before committing to a longer sequence. Check continuity, synchronization, artifacts and export playback. Provider access and rendering time vary.' },
  { title: '6. Prepare delivery', icon: Download, color: '#34d399', body: 'Download the actual returned files, reopen them outside the studio, and review on headphones and speakers. Check your distributor’s current specifications and retain rights documentation.' },
];
...
export default function BillboardBlueprintPage({ onBack }) {
  return (
    <main style={{ minHeight: '100vh', background: 'var(--color-bg, #11121b)', color: 'var(--text-primary, #f8fafc)', padding: '28px 20px 64px' }}>
      <div style={{ maxWidth: 1120, margin: '0 auto' }}>
        ...
        <header style={{ maxWidth: 800, marginBottom: 40 }}>
          <p style={{ color: '#a78bfa', fontWeight: 700, letterSpacing: '.12em', textTransform: 'uppercase' }}>Release blueprint</p>
          <h1 style={{ fontSize: 'clamp(2.2rem, 5vw, 4rem)', lineHeight: 1.12, margin: '12px 0 20px' }}>Make a record worth releasing.</h1>
          <p style={{ color: 'var(--text-secondary, #cbd5e1)', fontSize: '1.15rem', lineHeight: 1.75 }}>
          ...
          <div role="note" style={{ padding: 20, borderRadius: 16, border: '1px solid #a78bfa66', background: '#a78bfa12', lineHeight: 1.7 }}>
        ...
        <section aria-label="Production workflow" style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(min(100%, 300px), 1fr))', gap: 20 }}>
          {phases.map(({ title, icon: Icon, color, body }) => (
            <article key={title} style={{ padding: 24, borderRadius: 20, background: 'var(--color-bg-secondary, #1b1d2b)', border: '1px solid var(--border-color, #35384b)', borderTop: `3px solid ${color}` }}>
              <Icon size={25} color={color} aria-hidden="true" />
              <h2 style={{ fontSize: '1.2rem', margin: '16px 0 10px' }}>{title}</h2>
              <p style={{ color: 'var(--text-secondary, #cbd5e1)', lineHeight: 1.75, margin: 0 }}>{body}</p>
            </article>
          ))}
        </section>
        <section aria-labelledby="release-checks" style={{ marginTop: 36, padding: '28px clamp(16px, 4vw, 36px)', borderRadius: 20, border: '1px solid var(--border-color, #35384b)', background: 'var(--color-bg-secondary, #1b1d2b)' }}>
          ...
          <ul style={{ paddingLeft: 22, lineHeight: 1.8, color: 'var(--text-secondary, #cbd5e1)' }}>
